...
First, we import all spreadsheets for all conditions intended to be run. We add the condition variable to each, for identifiability once they are joined.

For each spreadsheet, we additionally check against our counterbalancing variables for that experiment and condition. 

```{r}
d.001 <- read.csv("../Spreadsheets/PACT-1a-001.csv", na.strings=c("", " ", "NA")) %>%
  mutate(condition = "1a-001")
unique(d.001$dose) #check that Dose, Test, Bias.Joanne, and Button1 are all correct
